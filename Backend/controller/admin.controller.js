...
import { uploadVideoToBlob ,uploadImageToBlob} from "../middleware/azureBlobService.js";
...
export async function uploadVideo(req, res) {
    ...
    const videoFile = req.files.video?.[0];  // Accessing the video file
    const imageFile = req.files.image?.[0];  // Accessing the image file (optional)

    // Validation for required fields
    if (!title || !description || !videoFile) {
        return res.status(400).json({ success: false, message: "Title, description, and video are required" });
    }

     // Validate price field (it should be 'free' or 'paid')
     if (type && !['entertainment', 'learning'].includes(type)) {
        return res.status(400).json({ success: false, message: "Price must be 'entertainment ' or 'learning'" });
    }

    // Set default price to 'free' if not provided
    const videoType = type || 'entertainment';

    // Default image URL in case no image is uploaded
    let imageUrl = 'https://setflixstorageaccount1.blob.core.windows.net/images/Video.jpg'; // Default image URL

    // Generate unique video and image names
    const videoName = title || `${Date.now()}_${videoFile.originalname}`;
    
    
    // Upload video to Azure Blob Storage and retrieve the URL
    const videoUrl = await uploadVideoToBlob(videoFile.buffer, videoName);

    // If an image is provided, upload it and retrieve the image URL
    if (imageFile) {
        const imageName = title || `${Date.now()}_${imageFile.originalname}`;
        imageUrl = await uploadImageToBlob(imageFile.buffer, imageName);
    }

    try {
        // Create a new video entry in the database with the video URL and image URL
        const newVideo = new Movie({
            name: videoName,
            description: description,
            videoUrl: videoUrl,
            image: imageUrl,  // Use the default or uploaded image URL
            type: videoType,
        });

        // Save the new video entry
        await newVideo.save();
